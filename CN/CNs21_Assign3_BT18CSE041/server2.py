...
def handleClient(conn, addr, client_num):
    connected = True
    while connected:
        ...
        if msg_length != '0':
            ...
            if msg == DISCONNECT_MESSAGE:
                ...
                global client_count
                print(f"Client socket {client_num} sent message: {msg}")
                ans = solve(msg)
...
try:
    ...
    while True:
        ...
        # create child thread to receive and respond to client's messages
        thread = threading.Thread(target=handleClient,args=(conn, addr, client_count))
        thread.start()
